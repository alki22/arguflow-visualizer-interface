    ...
    if (activeTab === 'argumentative-structure-analysis') {
      ...
      try {
        ...
        if (!premiseClaimResponse1.ok || !premiseClaimResponse2.ok) {
          throw new Error(`Premise-claim extraction failed`);
        }
        ...
        if (premiseClaimData1.status !== 'success' || premiseClaimData2.status !== 'success') {
          throw new Error("Failed to extract premise and claim");
        }
        
        const premise1 = premiseClaimData1.result.premise;
        const claim1 = premiseClaimData1.result.claim;
        const premise2 = premiseClaimData2.result.premise;
        const claim2 = premiseClaimData2.result.claim;
        
        // Extract topics for premises and claims
        const [premiseTopicResponse1, claimTopicResponse1, premiseTopicResponse2, claimTopicResponse2, argumentTopicResponse1, argumentTopicResponse2] = await Promise.all([
          fetch(API_ENDPOINTS['extract-topics'], {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ argument: premise1 }),
          }),
          fetch(API_ENDPOINTS['extract-topics'], {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ argument: claim1 }),
          }),
          fetch(API_ENDPOINTS['extract-topics'], {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ argument: premise2 }),
          }),
          fetch(API_ENDPOINTS['extract-topics'], {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ argument: claim2 }),
          }),
          fetch(API_ENDPOINTS['extract-topics'], {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        // Get stance and reasoning type for original arguments
        const [stanceResponse1, stanceResponse2, reasoningResponse1, reasoningResponse2] = await Promise.all([
          fetch(API_ENDPOINTS['stance-classification'], {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ argument1: text1, argument2: argumentTopicResponse1.ok ? (await argumentTopicResponse1.json()).result?.topics?.[0] || 'general' : 'general' }),
          }),
          fetch(API_ENDPOINTS['stance-classification'], {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ argument1: text2, argument2: argumentTopicResponse2.ok ? (await argumentTopicResponse2.json()).result?.topics?.[0] || 'general' : 'general' }),
          }),
          fetch(API_ENDPOINTS['reasoning-type-classification'], {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ argument1: text1 }),
          }),
          fetch(API_ENDPOINTS['reasoning-type-classification'], {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ argument1: text2 }),
          })
        ]);
        
        // Parse all responses
        const topicResponses = await Promise.all([
          premiseTopicResponse1.ok ? premiseTopicResponse1.json() : null,
          claimTopicResponse1.ok ? claimTopicResponse1.json() : null,
          premiseTopicResponse2.ok ? premiseTopicResponse2.json() : null,
          claimTopicResponse2.ok ? claimTopicResponse2.json() : null,
          argumentTopicResponse1.ok ? argumentTopicResponse1.json() : null,
          argumentTopicResponse2.ok ? argumentTopicResponse2.json() : null
        ]);
        
        const analysisResponses = await Promise.all([
          stanceResponse1.ok ? stanceResponse1.json() : null,
          stanceResponse2.ok ? stanceResponse2.json() : null,
          reasoningResponse1.ok ? reasoningResponse1.json() : null,
          reasoningResponse2.ok ? reasoningResponse2.json() : null
        ]);
        
        // Build result object
        const argumentativeResult = {
          argument1: {
            premise: premise1,
            claim: claim1,
            originalArgument: text1,
            premiseTopic: topicResponses[0]?.result?.topics?.[0] || 'N/A',
            claimTopic: topicResponses[1]?.result?.topics?.[0] || 'N/A',
            argumentTopic: topicResponses[4]?.result?.topics?.[0] || 'N/A',
            stance: analysisResponses[0]?.result?.stance || 'N/A',
            reasoningType: analysisResponses[2]?.result?.reasoning_type || 'N/A'
          },
          argument2: {
            premise: premise2,
            claim: claim2,
            originalArgument: text2,
            premiseTopic: topicResponses[2]?.result?.topics?.[0] || 'N/A',
            claimTopic: topicResponses[3]?.result?.topics?.[0] || 'N/A',
            argumentTopic: topicResponses[5]?.result?.topics?.[0] || 'N/A',
            stance: analysisResponses[1]?.result?.stance || 'N/A',
            reasoningType: analysisResponses[3]?.result?.reasoning_type || 'N/A'
          }
        };
        
        setResult(argumentativeResult);
